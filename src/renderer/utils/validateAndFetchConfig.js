...
const validateAndFetchConfig = async (configLocal, configLocalPath, setConfigRemote, setSelectedGame, setSelectedLanguage, setCurrentLocale, setLocale, setIsInitialized, setSelectedVoicePack, isInitialized, selectedGame, currentLocale) => {
  try {
    ...
    const activeGames = (remoteConfig?.games && remoteConfig.games.length > 0)
      ? remoteConfig.games
      : [{ name: "neo", clientVer: 0, patchVer: 0 }];

    if (!remoteConfig.games || remoteConfig.games.length === 0) {
      remoteConfig.games = activeGames;
    }

    const defaultGameName = activeGames[0]?.name || "neo";
    if (configLocal?.selectedGame?.length > 0 && activeGames.some(g => g.name === configLocal.selectedGame)) {
      setSelectedGame(configLocal.selectedGame);
    } else {
      updateConfigJson("selectedGame", defaultGameName, configLocalPath);
      setSelectedGame(defaultGameName);
    }
