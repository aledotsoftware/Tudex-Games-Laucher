      ...
      <div className="initial-setup">
        <div className="initial-setup-card">
          <div className="initial-setup-header">
            <div className="initial-setup-logo" />
            <h3 className="initial-setup-title">Tudex Games Launcher</h3>
          </div>
          <span className="initial-setup-text">
            {getMessages(currentLocale).initializing}
          </span>
          <div className="initial-setup-bar-container">
            <div className="initial-setup-bar-fill" />
          </div>
          <div className="initial-setup-footer">
            <span>v{configLocal?.launcherVer || 1}</span>
          </div>
        </div>
      </div>
      <div className="launcher-version-tag">
        v{configLocal?.launcherVer || 1}
      </div>
      <div className="side-menu">
        {configRemote?.games?.map((game) => (
          <div
            key={game?.name}
            className={`game-icon ${game?.name?.toLowerCase()} ${selectedGame === game?.name ? "selected" : ""
              }`}
            style={game?.iconUrl ? {
      ...
      <div className="patcher">
        {configRemote?.games?.map((game) => (
          <div
            ...
            className={`game-patcher ${game?.name?.toLowerCase()} ${selectedGame === game?.name ? "active" : ""
              }`}
            style={game?.backgroundUrl ? {
              backgroundImage: `url(${game.backgroundUrl})`,
